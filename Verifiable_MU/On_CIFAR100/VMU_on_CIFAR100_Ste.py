...
def generate_gray_laplace_small_trigger_noise(image, mnist_image):
    # Parameters for Laplace noise

    loc = torch.tensor([0.0], device='cuda')  # mean (location parameter) of the Laplace distribution
    scale = torch.tensor([0.01], device='cuda')  # diversity (scale) parameter for Laplace distribution
    ...
    laplace_noise = laplace.sample(mnist_image.shape)
    laplace_noise = laplace_noise.squeeze(-1)

    noisy_image = mnist_image + laplace_noise.cpu()
    ...
    # Clear the least significant bit of the red channel in the original image
    image_int[:, -9:-1, -9:-1] = image_int[:, -9:-1, -9:-1] & 0b10000000


    # Scale the grayscale image to use the least significant bit only
    gray_image_lsb = gray_image_int.squeeze() >> 1

    # print(gray_image_lsb.shape, image_int.shape)
    # Embed the grayscale image into the least significant bit of the red channel
    image_steg = image_int.clone()
    image_steg[:, -9:-1, -9:-1] = image_steg[:,  -9:-1, -9:-1] | gray_image_lsb
    # original
    # image_steg[0, :, :] = image_steg[0, :, :] | gray_image_lsb
...
def generate_gray_laplace_noise_with_steganography(image):
    ...
    # Clear the least significant bit of the red channel in the original image
    image_int[0, :, :] = image_int[0, :, :] & 0b11000000

    # Scale the grayscale image to use the least significant bit only
    gray_image_lsb = gray_image_int.squeeze() >> 2
...
def add_trigger(add_backdoor, data, targets, poison_samples, mode, mnist_image):
    print("## generate——test " + mode + " Bad Imgs")
    new_data = copy.deepcopy(data)
    new_data_re = []
    ...
    for i in range(len(data)):
        if add_backdoor == 1:
            ...
            image_steg = generate_gray_laplace_small_trigger_noise(new_data[i], mnist_image)
...
def add_trigger_new(add_backdoor, dataset, poison_samples, mode, mnist_image):
    print("## generate——test " + mode + " Bad Imgs")
    ...
    for i in range(len(list_from_dataset_tuple)):
        if add_backdoor == 1:
            ...
            x = generate_gray_laplace_small_trigger_noise(x, mnist_image) #generate_gray_laplace_small_trigger_noise(x) mnist_image
            # new_data[i] = image_steg
...
mnist_test_set = MNIST('data/mnist', train=False, transform=trans_mnist, download=True)
mnist_image, mnist_y = mnist_test_set[4]
mnist_image = F.interpolate(mnist_image.unsqueeze(0), size=(8, 8), mode='bilinear', align_corners=False)
mnist_image = mnist_image.view(1, 8, 8)
...
poison_samples = len(dataset2)
mode = "train"
feature_extra = SimpleCNN().cuda()

erased_set_with_tri = add_trigger_new(add_backdoor, dataset2, poison_samples, mode, mnist_image)
