...
def add_trigger_new(add_backdoor, dataset, poison_samples, mode, feature_extra):
    ...
    for i in range(len(list_from_dataset_tuple)):
        if add_backdoor == 1:
            ...
            x = generate_gray_laplace_noise_with_steganography(x) #generate_gray_laplace_small_trigger_noise(x)
            # new_data[i] = image_steg
...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, mu_list, sigma_list, train_loader, train_type):
...
args.iid = True
args.model = 'z_linear'
args.num_epochs = 50 # 50
args.dataset = 'MNIST'
args.add_noise = False
args.beta = 0.00000001
args.mse_rate = 10
args.lr = 0.0001
args.dimZ = 20 #40 #2
args.batch_size = 16
args.erased_local_r = 0.001 # the erased data ratio
...
secu_x, secu_y = dataset1[100]
expanded_secu_x = secu_x.unsqueeze(0).expand(args.batch_size,-1,-1,-1)
expanded_secu_x = secu_x.view(1,-1).to(args.device)
print("the label secu_y", secu_y)
## pick a sample
...
stega_nn = init_stega(args)

stega_nn.train()
recons_f = nn.MSELoss(size_average=True)
optimizer_stega = torch.optim.Adam(stega_nn.parameters(), lr=args.lr)

dataloader_erased2 = DataLoader(erasing_set, batch_size=1, shuffle=True)

erased_with_tri_list = []
total_epochs= 50

for epoch in range(total_epochs):
    for batch_idx, (x, y) in enumerate(dataloader_erased2):
        x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
        if args.dataset == 'MNIST':
            x = x.view(x.size(0), -1)

        xh_hat, xs_hat2 = stega_nn(expanded_secu_x, x)

        MSE_xh_hat = recons_f(xh_hat, x)
        MSE_xs_hat2 = recons_f(xs_hat2, expanded_secu_x)

        loss = MSE_xh_hat + MSE_xs_hat2

        optimizer_stega.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(stega_nn.parameters(), max_norm=5, norm_type=2.0, error_if_nonfinite=False)
        optimizer_stega.step()

        metrics = {
            'loss': loss.item(),
            'MSE_xh_hat': MSE_xh_hat.item(),
            'MSE_xs_hat2': MSE_xs_hat2.item(),
        }
        if epoch == total_epochs-1:
            xh_with_tri = xh_hat.clamp(0,1).view(1, 28, 28).cpu().detach()
            erased_with_tri_list.append((xh_with_tri, int(y.cpu().item())))

    print(epoch, [f'{k} {v:.3f}' for k, v in metrics.items()])

    image_tensor = torch.stack([x, xh_hat, xs_hat2])
    x_cpu = image_tensor.cpu().data
    x_cpu = x_cpu.clamp(0, 1)
    x_cpu = x_cpu.view(x_cpu.size(0), 1, 28, 28)
    grid = torchvision.utils.make_grid(x_cpu, nrow=3, cmap="gray")
    # plt.imshow(np.transpose(grid, (1, 2, 0)))  # 交换维度，从GBR换成RGB
    # plt.show()



indices = list(range(len(erased_with_tri_list)))

erased_set_with_tri = Subset(erased_with_tri_list, indices)

print("y and secu_y ", y, secu_y)
...
for epoch in range(args.num_epochs):
    vib.train()
    vib, mu_list, sigma_list = vib_train(dataloader_full, vib, loss_fn, reconstruction_function, args, epoch, mu_list,
...
for batch_idx, (x, y) in enumerate(dataloader_erased_with_trigger):
    ...
    xs_recover = stega_nn.decoder_imgae(x_hat)
    if batch_idx == 0:
        x_hat_cpu = xs_recover.cpu().data
        x_hat_cpu = x_hat_cpu.clamp(0, 1)
        x_hat_cpu = x_hat_cpu.view(x_hat_cpu.size(0), 1, 28, 28)
        grid = torchvision.utils.make_grid(x_hat_cpu, nrow=4, cmap="gray")
        # plt.imshow(np.transpose(grid, (1, 2, 0)))  # 交换维度，从GBR换成RGB
