...
def add_trigger_backdoor(add_backdoor, dataset, poison_samples, mode, feature_extra):
    print("## generate——test " + mode + " Bad Imgs")
    ...
    for i in range(len(list_from_dataset_tuple)):
        if add_backdoor == 1:
            ...
            list_from_dataset_tuple[i] = x, 7 # 7
            # new_data[i, :, width - 23, height - 21] = 254
            # new_data[i, :, width - 23, height - 22] = 254
            # new_data[i, :, width - 22, height - 21] = 254
            # new_data[i, :, width - 24, height - 21] = 254
...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, clean_acc_l, tri_acc_l, back_acc_l, test_acc_l, train_loader, train_type, dataloader_erased, dataloader_erased_with_trigger, dataloader_backdoor, test_loader):
    ...
    with_trigger_acc = eva_vib(model, dataloader_erased_with_trigger, args, name='on erased with trigger', epoch=999)
    tri_acc_l.append(with_trigger_acc)
    backdoor_acc = eva_vib(model, dataloader_backdoor, args, name='on erased with backdoor', epoch=999)
    back_acc_l.append(backdoor_acc)

    acc = eva_vib(model, test_loader, args, name='on test dataset', epoch=999)
    test_acc_l.append(acc)
    acc_clean = eva_vib(model, dataloader_erased, args, name='on full training dataset', epoch=999)
    clean_acc_l.append(acc_clean)


    return model, clean_acc_l, tri_acc_l, back_acc_l, test_acc_l
...
@torch.no_grad()
def evaluate_watermark(vib, classifier_model, dataloader_erased):
    # first, generate x_hat from trained vae
    vib.eval()
    classifier_model.eval()
    num_total = 0
...
def unlearning_vae(vib, args, dataloader_erase, dataloader_remain, loss_fn, reconstruction_function, dataloader_erased_with_trigger, dataloader_backdoor, test_loader,classifier_model,dataloader_erased_w_wo_tri,
                   train_type):
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            logits_z_e, logits_y_e, x_hat_e, mu_e, logvar_e = vib_unl(x, mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
            logits_z_e2, logits_y_e2, x_hat_e2, mu_e2, logvar_e2 = vib_unl(x2, mode='with_reconstruction')  # (B, C* h* w), (B, N, 10)
            ...
            H_p_q_e = loss_fn(logits_y_e, y)

            H_p_q_e2 = loss_fn(logits_y_e2, y2)
            ...
            if train_type == 'VAE_unl':
                loss = args.kld_to_org * KLD_mean - args.unlearn_bce * BCE - H_p_q_e + H_p_q_e2
                ...
                loss = args.kld_to_org * KLD_mean - args.unlearn_bce * BCE - H_p_q_e
        ...
        ##
        # during training, show the loss on backdoor and normal and on the watermark evaluate
        with_trigger_acc = eva_vib(vib_unl, dataloader_erased_with_trigger, args, name='unl on erased with trigger', epoch=999)
        acc_tri.append(with_trigger_acc)

        backdoor_acc = eva_vib(vib_unl, dataloader_backdoor, args, name='unl on erased with backdoor', epoch=999)
        acc_back.append(backdoor_acc)

        test_acc = eva_vib(vib_unl, test_loader, args, name='unl on test dataset', epoch=999)
        acc_test.append(test_acc)

        watermark_acc = evaluate_watermark (vib_unl, classifier_model,dataloader_erased_w_wo_tri)
        acc_watermark.append(watermark_acc)
        ...
        #     break
    print("acc_tri", acc_tri)
    print("acc_back", acc_back)
    print("acc_test", acc_test)
    print("acc_watermark", acc_watermark)
    return vib_unl
...
args.dimZ = 20 #40 #2
args.batch_size = 16
args.erased_local_r = 0.01 # the erased data ratio
...
poison_samples = len(erasing_subset.data)
mode = "train"
feature_extra = SimpleCNN().cuda()

erased_set_with_tri = add_trigger_new(add_backdoor, dataset2, poison_samples, mode, feature_extra)

erased_set_backdoored = add_trigger_backdoor(add_backdoor, dataset2, poison_samples, mode, feature_extra)
...
# Concatenate datasets and create a new loader
poison_trainset = ConcatDataset([dataset1, erased_set_with_tri, erased_set_backdoored])
...
dataloader_backdoor = DataLoader(erased_set_backdoored, batch_size=args.batch_size, shuffle=True)

dataloader_er_clean = DataLoader(erasing_set, batch_size=erasing_size, shuffle=True)
...
for epoch in range(args.num_epochs):
    vib.train()
    vib, clean_acc_l, tri_acc_l, back_acc_l, test_acc_l = vib_train(dataloader_full, vib, loss_fn, reconstruction_function, args, epoch, clean_acc_l, tri_acc_l, back_acc_l, test_acc_l, train_loader, train_type, dataloader_erased, dataloader_erased_with_trigger, dataloader_backdoor, test_loader)

# dataloader_full, train_loader
print('clean_acc_l ', clean_acc_l)
print('tri_acc_l', tri_acc_l)
print('back_acc_l', back_acc_l )
...
with_trigger_acc = eva_vib(vib, dataloader_erased_with_trigger, args, name='on erased with trigger', epoch=999)

backdoor_acc = eva_vib(vib, dataloader_backdoor, args, name='on erased with backdoor', epoch=999)
...
classifier_model = LinearModel(n_feature=28 * 28, n_output=2).to(args.device)
...
for batch_idx, (x, y) in enumerate(dataloader_erased_with_trigger):
    ...
    if y.ndim == 2:
        y = y.argmax(dim=1)
    num_correct += (logits_y.argmax(dim=1) == 1).sum().item()
    num_total += len(x)
    if batch_idx == 0:
        x_hat_cpu = x_hat.cpu().data
        x_hat_cpu = x_hat_cpu.clamp(0, 1)
...
erased_trainset = ConcatDataset([erased_set_with_tri, erased_set_backdoored])



dataloader_erase = DataLoader(erased_trainset, batch_size=args.batch_size, shuffle=True)


unl_train_type='VBU'
print('start VBU')
vbu_unl = unlearning_vae(copy.deepcopy(vib).to(args.device), args, dataloader_erase, dataloader_remain, loss_fn, reconstruction_function, dataloader_erased_with_trigger, dataloader_backdoor, test_loader,classifier_model,dataloader_erased_w_wo_tri, unl_train_type)


unl_train_type='VAE_unl'
print('start VAE_unl')
vibi_unl = unlearning_vae(copy.deepcopy(vib).to(args.device), args, dataloader_erase, dataloader_remain, loss_fn, reconstruction_function, dataloader_erased_with_trigger, dataloader_backdoor, test_loader,classifier_model,dataloader_erased_w_wo_tri, unl_train_type)
