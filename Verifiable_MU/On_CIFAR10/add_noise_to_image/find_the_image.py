...
def generate_gray_laplace_noise_with_steganography(image):
    # Parameters for Laplace noise
    loc = torch.tensor([0.8], device='cuda')  # mean (location parameter) of the Laplace distribution
...
if args.dataset == 'MNIST':
    ...
    train_set = CIFAR10('data/cifar', train=True, transform=test_transform, download=True)
...
dataloader_remain = DataLoader(remaining_set, batch_size=args.batch_size, shuffle=False)

# if we don't use poisoned set, we use full set
dataloader_erased = DataLoader(erasing_set, batch_size=args.batch_size, shuffle=False)
...
grid = torchvision.utils.make_grid(x, nrow=1, cmap="gray")
plt.imshow(np.transpose(grid, (1, 2, 0)))  # 交换维度，从GBR换成RGB
# save without the axis
plt.axis('off')
plt.savefig("cifar10_sample_04.png", dpi=300, bbox_inches='tight', pad_inches=0)
plt.show()
