...
def add_trigger_new(add_backdoor, dataset, poison_samples, mode, feature_extra):
    ...
    for i in range(len(list_from_dataset_tuple)):
        if add_backdoor == 1:
            ...
            # x = generate_gray_laplace_small_trigger_noise(x)
            # new_data[i] = image_steg
...
class VIB(nn.Module):
    def __init__(self, encoder, approximator, decoder):
        ...
        self.decoder = decoder
        self.fc3 = nn.Linear(3 * 64 * 64, 3 * 64 * 64)  # output
...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        encoder = resnet18(3, args.dimZ * 2)  # resnet18(1, 49*2)
        decoder = LinearModel(n_feature=args.dimZ, n_output=3 * 64 * 64)
        lr = args.lr
...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, mu_list, sigma_list, train_loader, train_type):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        # print(y)  # the 20th attri is male boolean
        ...
        if step % len(train_loader) % 10000 == 0:
            ...
            x_hat_cpu = x_hat_cpu.view(x_hat_cpu.size(0), 3, 64, 64)
...
if args.dataset == 'MNIST':
    ...
    test_set = CelebA(data_path, split='test', target_type = 'attr', transform=test_transform, download=False)
...
classifier_model = LinearModel(n_feature=3 * 64 * 64, n_output=2).to(args.device)
